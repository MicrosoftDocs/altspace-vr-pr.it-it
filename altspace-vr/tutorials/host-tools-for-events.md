...
---
# <a name="host-tools-for-frontrow-events"></a>Strumenti host per eventi FrontRow

Questo articolo si basa sulla presenza di FrontRow abilitato per l'evento. Per abilitare FrontRow per l'evento, vedere questo articolo: [Ridimensionamento dei destinatari con la funzionalità FrontRow](../faqs/scaling-audiences.md).

Per altre informazioni sugli strumenti host per un evento non FrontRow, vedere Panoramica [degli strumenti host](host-tools-overview.md)
...
Di seguito è riportata una panoramica dettagliata di queste funzionalità, capacità e altri utili suggerimenti per l'hosting di eventi FrontRow in AltspaceVR.
...
*È possibile indicare che l'evento è impostato su FrontRow se viene visualizzato il pulsante GO ON-AIR.*

### <a name="toggle-mute-all"></a>Attiva/disattiva Mute-All
...
Gli host possono inviare un messaggio a tutti i partecipanti presenti nell'evento FrontRow. Usare una delle note rapide o scriverne una originale. Dopo l'invio, tutti gli utenti riceveranno il messaggio come popup.

### <a name="amplify-my-voice"></a>Amplificare la voce

Gli host possono amplificare i microfoni per assicurarsi che tutti gli utenti dell'evento in prima riga ascoltino ciò che si sta pronunciando indipendentemente dalla distanza.

### <a name="go-on-air"></a>Go On-Air

Tutti gli utenti con il ruolo Host potranno visualizzare questo pulsante. Se si fa clic su Go On-Air, l'avatar verrà "rispecchiato" in tutte le stanze frontrow. Dopo aver "con mirroring" tutto ciò che si dice e si fa verrà visualizzato in tutte le istanze dell'evento FrontRow.

Essere in onda per l'intera durata dell'evento FrontRow è MOLTO IMPORTANTE. Per il corretto funzionamento degli strumenti host in tutte le stanze frontrow, è necessario che gli strumenti host funzionino correttamente in tutte le stanze frontrow. In caso contrario, funzionano solo nella stanza in cui ci si sta.

> [!NOTE]
> Se è necessario ricongiungersi all'evento, passare al Campofire e tornare indietro, perdere la connessione Internet, passare da Stanza a Stanza, quando si torna non si sarà in diretta. Assicurarsi di fare di nuovo clic su Go On-Air una volta tornati.
...
## <a name="the-host-panel"></a>Pannello host
...
L'host eventi è responsabile della gestione dell'esperienza di altri utenti. Per moderare in modo efficiente, l'utente e i moderatori hanno bisogno di strumenti di moderazione affidabili. In quanto host di un evento FrontRow, è possibile disattivare gli utenti che causano interruzioni dell'evento, escludere le regole dall'evento (temporaneamente o durante l'evento) e persino assegnare il ruolo Moderator/Host ad altri utenti durante l'evento per ottenere assistenza nella gestione dei destinatari.
...
*Fare clic sul menu hamburger per aprire tutte le opzioni dello strumento di moderazione.*
...
A questo punto è possibile lavorare con FrontRow Hosting Magic e usare queste funzionalità nei partecipanti:
    ...
    * **Rendi host:** assegna loro il ruolo Host nell'evento FrontRow. Aggiungere un Co-Host in tempo reale, assicurarsi che siano disponibili in tempo reale e che possano aiutare a eseguire le operazioni o a farlo recare nelle altre stanze per verificare le cose. Tutti gli utenti sono in modalità on-air e vengono visualizzati nelle stanze secondarie? La presentazione è sincronizzata in tutte le stanze attive? Fare in modo che siano in onda quando si sono nell'altra stanza e si sarà in grado di parlare dell'avatar con l'avatar da Stanza a Stanza.
...
*Se si fa clic su "Alza mano", i guest vengono in una coda che si controlla.*

Quando un guest fa clic su questo pulsante, viene visualizzato nel pannello di partecipazione:

![Pannello di partecipazione aperto con guest con la mano alzata in coda evidenziata](images/host-front-row-img-07.png)

Per tenere traccia, il pannello di partecipazione accenderà anche una notifica rossa quando si hanno ospiti in attesa di essere chiamati e sarà possibile sapere per quanto tempo ogni ospite è stato in attesa.
...
Esaminare rapidamente le informazioni o i pulsanti necessari per gestire Le stanze durante l'evento FrontRow:

![Scheda del pannello Rooms aperta dal pannello host](images/host-front-row-img-08.png)

*Si noti che **indica (You're Here)**, ovvero la stanza in cui ci si trova attualmente.*

Le stanze hanno pulsanti per Reimposta spazio, Elimina spazio, Ridistribuire gli utenti e Teletrasporta qui:
...
A volte, dopo che l'evento FrontRow è stato fatto per un po' di tempo, le prime camere hanno quasi la capacità, ma l'ultima camera o due può avere solo un numero ridotto di ospiti per stanza. Facendo clic su questo pulsante, gli ospiti in tale stanza possono scegliere di rimanere nella stanza corrente o di andare in una stanza più popolata.
...
Ti senti inviviato a provare tutto? Ottimo. Andare avanti ed estendere questi elementi host di eventi FrontRow.
